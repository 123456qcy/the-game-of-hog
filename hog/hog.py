...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            ...
            return announce_highest(0, score0, max(score0 - last_score, running_high))
        else:
            if score1 - last_score > running_high:
                print(score1 - last_score, "point(s)! The most yet for Player 1")
            return announce_highest(1, score1, max(score1 - last_score, running_high))
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    "*** YOUR CODE HERE ***"
    max = 1
    roll_max = make_averaged(roll_dice, trials_count)
    roll_max_score = roll_max(1, dice)
    for i in range(2, 11):
        roll_i = make_averaged(roll_dice, trials_count)
        roll_i_score = roll_i(i, dice)
        if roll_i_score > roll_max_score:
            max = i
            roll_max_score = roll_i_score
    return max
    # END PROBLEM 9
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    #max_num_rolls = max_scoring_num_rolls(six_sided)不让用这个函数
    if score + free_bacon(opponent_score) >= GOAL_SCORE:
        return 0#如果free_bacon直接赢了就不用roll了
    if score >= 90 and score - opponent_score > 10:
        return 2
